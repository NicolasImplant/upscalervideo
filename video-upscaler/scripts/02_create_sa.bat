...
if errorlevel 1 (
    echo   Creando SA...
    call gcloud iam service-accounts create %SA_NAME% --display-name="Video Upscaler Job SA" --project=%GCP_PROJECT_ID% --quiet
...
call gcloud storage buckets add-iam-policy-binding gs://%GCS_BUCKET_INPUT% --member="serviceAccount:%SA_EMAIL%" --role="roles/storage.objectViewer" >nul 2>&1
if errorlevel 1 ( echo   [WARN] Bucket input no existe aun. Ejecuta 03 primero. ) else ( echo   [OK] objectViewer en input. )
call gcloud storage buckets add-iam-policy-binding gs://%GCS_BUCKET_OUTPUT% --member="serviceAccount:%SA_EMAIL%" --role="roles/storage.objectCreator" >nul 2>&1
if errorlevel 1 ( echo   [WARN] Bucket output no existe aun. Ejecuta 03 primero. ) else ( echo   [OK] objectCreator en output. )
call gcloud projects add-iam-policy-binding %GCP_PROJECT_ID% --member="serviceAccount:%SA_EMAIL%" --role="roles/logging.logWriter" --quiet >nul 2>&1
